...
def check_prerequisites():
    ...
    for tool, command in checks.items():
        ...
        result = run_command(" ".join(command) if platform.system() == "Windows" else command, f"Check {tool}", check=False)
        results[tool] = result
...
def check_gcloud_sdk():
    ...
    if run_command("gcloud version", "Check Google Cloud SDK", check=False):
        ...
        if platform.system() == "Windows":
            print("  1. Download from: https://cloud.google.com/sdk/docs/install-sdk#windows")
            print("  2. Run the installer")
            print("  3. Restart PowerShell/Command Prompt")
        elif platform.system() == "Darwin":  # macOS
            print("  1. Using Homebrew: brew install google-cloud-sdk")
            print("  2. Or download from: https://cloud.google.com/sdk/docs/install-sdk#mac")
        else:  # Linux
            print("  1. Follow instructions at: https://cloud.google.com/sdk/docs/install-sdk#linux")
...
def main():
    ...
    missing_tools = [tool for tool, installed in prereq_results.items() if not installed]
    if missing_tools:
